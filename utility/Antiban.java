...
public class Antiban {
	...
	/**
	 * Updates boolean only if resources are not available.
	 * 
	 * @param id of the resources to look
	 */
	public void setShouldMoveToAnticipated(int...id) {
		if (Objects.findNearest(10, id).length == 0)
			this.shouldMoveToAnticipated = abcInstance.shouldMoveToAnticipated();
	}
	
	/**
	 * Moves to anticipated location if it should.
	 * 
	 * @param target object, used to get location
	 */
	public void executeMoveToAnticipated() {
		if(this.shouldMoveToAnticipated) {
			RSObject [] treeStump = Objects.findNearest(5, "Tree Stump");
			if(treeStump.length <= 1) {
				print("No anticipated target found");
				return;
			}
			print("Moving to anticipated target");
			Walking.blindWalkTo(treeStump[1].getPosition());
		} else {
			print("The bot should not move to anticipated");
		}
	}
